...
clear all
close all
NumberOfUsers = 3;% please enter number of user equipments
B = 50*10^6;% Hz 
Pt = 1; %watt
N0 = 10^(-17);    %noise density watt/Hz (-210 dBm/Hz)
PowerMatrix = dlmread('power_user3.txt',' ');
powerArray = zeros(length(PowerMatrix), NumberOfUsers);
%free space loss dB for 2 GHz
distanceMatrix = dlmread('distance_user3.txt',' ');
...
%fprintf('user distance data rate power coefficient\n');
 comb=[];
for d = 1:1:length(distanceMatrix)
    distance = zeros(1,NumberOfUsers);
    %distance(1) = 318;
    %distance(2) = 125;
    distance(1) = distanceMatrix(d,1);
    distance(2) = distanceMatrix(d,2);
    %distance(3) = distanceMatrix(d,3);
 
    d1 = distanceMatrix(d,1);
    d_inc = distanceMatrix(d,2);
    %d_inc2=(distanceMatrix(d,2)-distanceMatrix(d,1));
    fprintf('d1 = %d; d2 = %d;\n',d1, d_inc);
    ...
    %for i = 1: 1: NumberOfUsers
       %loss(i) = -20*log10(d1+d_inc*(i - 1)+(d_inc2*(2*i-1))./3) - 20*log10(f) + 147.55; %free space path loss
    %end
    for i = 1: 1: NumberOfUsers
    ...
    for a=1:1:NumberOfUsers
        fprintf("data rate: %.3f;", cap(max_sum_index,a));
        %(a+" "+distance(a)+" "+cap(max_sum_index,a)+" "+powerArray(max_sum_index,a)+'\n');
        fprintf('user %d distance: %d; data rate: %.3f; power coefficient: %.3f\n',a, distance(a), cap(max_sum_index,a),powerArray(max_sum_index,a)); 
        dis=[dis; distance(a)];
        pow=[pow; powerArray(max_sum_index,a)];
    end
  comb_1=[dis; pow];
  comb=[comb, comb_1];
  ...
  fileID = fopen('trainingData_3.txt','w');
  fprintf(fileID, 'Distance/Power\n\n');
  fprintf(fileID,'%i %i %.3f %.3f\n',comb);
  fclose(fileID);
end
