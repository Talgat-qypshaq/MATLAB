...
distanceMatrix = dlmread('data/distances_1.txt',' ');
trainingData_1 = zeros(length(distanceMatrix), 5);
...
for d = 1:1:length(distanceMatrix)            
    ...
    trainingData_1(d,1) = d1;
    trainingData_1(d,2) = d_inc;
    trainingData_1(d,3) = round(cap(max_sum_index,1)/1000000,2);
    trainingData_1(d,4) = round(cap(max_sum_index,2)/1000000,2);
    trainingData_1(d,5) = powerArray(max_sum_index,1);
    trainingData_2(d,1) = d1;
    trainingData_2(d,2) = d_inc;
    trainingData_2(d,3) = powerArray(max_sum_index,1);
...
writematrix(trainingData_1,'data/trainingData_1.txt','Delimiter',' ');
