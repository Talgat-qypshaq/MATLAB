...
for d = 1:1:length(distanceMatrix)
    ...
    trainingData(d,1) = d1;
    trainingData(d,2) = d_inc;
    trainingData(d,3) = round(cap(max_sum_index,1)/1000000,2);
    trainingData(d,4) = round(cap(max_sum_index,2)/1000000,2);
    trainingData(d,5) = powerArray(max_sum_index,1);
